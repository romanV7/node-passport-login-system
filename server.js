...
app.post('/login', checkNoAuthenticated, passport.authenticate('local', { successRedirect: '/', failureRedirect: '/login', failureFlash: true }))
...
app.post('/register', checkNoAuthenticated, async (req, res) => {
  try {
    ...
    users.push({ name, email, id: Date.now().toString(),password: hashedPassword })
    res.redirect('/login')
